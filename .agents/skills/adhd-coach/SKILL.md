...
- `vault/adhd/AGENTS.md` (behavioral rules that apply even without the skill)
...
The vault is at `vault/adhd/` relative to the repo root. After a vault move, either work from this repo with absolute paths, or copy the skill to `~/.Codex/skills/adhd-coach/`.

## What this skill is not

- Not therapy. Every session that surfaces therapeutic material ends with a "bring this to your therapist" note.
- Not the discernment coach. Discernment questions route to `.Codex/skills/discernment-coach/`. This skill reads `vault/discernment/` for commitment context via `foundations/commitments.md` but does not replace the discernment coach.
